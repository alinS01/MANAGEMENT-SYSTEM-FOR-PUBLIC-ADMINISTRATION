...
public class Pensionar extends Utilizator {
    ...
    public int exceptieCerere(Cerere cerere) throws IOException {
        ...
        if(cerere.getTipCerere().compareTo(autorizatie) == 0){
            try {
                throw new ExceptieCerere("Utilizatorul de tip pensionar nu poate inainta o cerere de tip: reinnoire autorizatie");
        ...
        if(cerere.getTipCerere().compareTo(carnetElev) == 0){
            try {
                throw new ExceptieCerere("Utilizatorul de tip pensionar nu poate inainta o cerere de tip: inlocuire carnet de elev");
            } catch (ExceptieCerere e) {
                ManagementPrimarie.scrieFisier(e.text);
                return 0;
            }
        ...
        if(cerere.getTipCerere().compareTo(actConstitutiv) == 0){
            try {
                throw new ExceptieCerere("Utilizatorul de tip pensionar nu poate inainta o cerere de tip: creare act constitutiv");
            } catch (ExceptieCerere e) {
                ManagementPrimarie.scrieFisier(e.text);
                return 0;
            }
        ...
        if(cerere.getTipCerere().compareTo(venitSalarial) == 0){
            try {
                throw new ExceptieCerere("Utilizatorul de tip pensionar nu poate inainta o cerere de tip: inregistrare venit salarial");
            } catch (ExceptieCerere e) {
                ManagementPrimarie.scrieFisier(e.text);
